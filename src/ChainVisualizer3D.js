  ...
  useEffect(() => {
    ...
    if (!initialCameraSetup && items.length > 0 && cameraRef.current && controlsReady && controlsRef.current) {
      ...
      if (blockPositions.length > 0) {
        ...
        // Position camera at a good distance to see everything
        const distanceMultiplier = Math.max(1.5, blockSpread / 300); // Scale based on how spread out blocks are
        const cameraX = centerX;
        const cameraY = Math.max(centerY + 200, maxY + 100); // Above the blocks
        const cameraZ = centerZ + 300 * distanceMultiplier; // Back far enough to see everything
        
        cameraRef.current.position.set(cameraX, cameraY, cameraZ);
        controlsRef.current.target.set(centerX, centerY, centerZ); // Look at center of blocks
        controlsRef.current.update();
        
        // Mark initial camera setup as complete
        setInitialCameraSetup(true);
        
        console.log(`📷 Initial camera positioned at (${cameraX.toFixed(1)}, ${cameraY.toFixed(1)}, ${cameraZ.toFixed(1)}) looking at block center (${centerX.toFixed(1)}, ${centerY.toFixed(1)}, ${centerZ.toFixed(1)})`);
      }
    }
    }, 100); // 100ms debounce
