...
export const useBlockchainData = () => {
  ...
  const fetchMissingParent = useCallback(async (parentHash) => {
    ...
    try {
      ...
      const httpUrl = 'https://node.quaiswap.io/cyprus1'; // Convert WebSocket URL to HTTP
  ...
  const pollLatestBlock = useCallback(async () => {
    ...
    try {
      const response = await fetch('https://node.quaiswap.io/cyprus1', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      try {
        const ws = new WebSocket('wss://node.quaiswap.io/ws/cyprus1');
